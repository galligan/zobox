...
/**
 * Schema for a sorter definition [sorters.<name>]
 * Additional metadata is allowed via catchAll
 */
export const SorterDefinitionSchema = z
  .object({
    type: z.string().min(1, "sorter type must not be empty"),
    description: z.string().optional(),
...
/**
 * Schema for a destination in routes.json
 */
...
/**
 * Schema for new item input via POST /messages
 */
