/**
 * Tests for POST /items handler decomposition.
 */
...
describe("parseMultipartRequest", () => {
  it("should parse valid multipart request with event field", async () => {
    const app = new Hono();
    const item = { type: "task", payload: { title: "Test" } };
    ...
    const _c = await Promise.resolve(app.request(req)).then(
      (_res: Response) => {
        ...
        ctx.req.parseBody = async () => {
          ...
          fd.append("event", JSON.stringify(item));
    ...
    expect(result.item.payload).toEqual({ title: "Test" });
    expect(result.attachments).toEqual([]);
  });

  it("should parse multipart request with attachments", async () => {
    const item = { type: "note", payload: { text: "Hello" } };
...
describe("parseJsonRequest", () => {
  it("should parse valid JSON request", async () => {
    ...
    expect(result.item.type).toBe("task");
    expect(result.item.payload).toEqual({ title: "Test task" });
    expect(result.item.channel).toBe("default");
    expect(result.attachments).toEqual([]);
  });
