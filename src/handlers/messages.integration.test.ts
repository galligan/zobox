/**
 * Integration tests for POST /messages handler with decomposed functions.
...
describe("POST /messages handler integration", () => {
  let tempDir: string;
  let runtime: RuntimeContext;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zobox-messages-test-"));
  ...
  it("should handle JSON request without attachments", async () => {
    ...
    const { message, attachments } = await parseRequest(mockContext);

    expect(message.type).toBe("task");
    expect(message.payload).toEqual({ title: "Test task", priority: "high" });
    expect(message.channel).toBe("work");
    ...
    // Verify envelope was written (inbox/YYYY-MM-DD/<id>.json)
