...
- [POST /messages](API.md#post-messages) - Ingest messages
- [GET /messages](API.md#get-messages) - List messages
- [GET /messages/next](API.md#get-messagesnext) - Worker polling
- [POST /messages/:id/ack](API.md#post-messagesidack) - Acknowledge processing
...
Store messages locally without routing:
...
Send messages to external webhooks:
...
Build distributed subscribers:

```bash
# Worker fetches unclaimed messages
curl "http://localhost:8787/messages/next?subscriber=worker-1"

# Worker acknowledges processing
curl -X POST "http://localhost:8787/messages/{id}/ack" \
  -d '{"subscriber":"worker-1"}'
```

See: [GET /messages/next](API.md#get-messagesnext), [POST /messages/:id/ack](API.md#post-messagesidack)
...
See: [POST /messages - Mode C](API.md#post-messages---mode-c-multipart-form-data)
