...
The `[files]` section in `zobox.config.toml` defines global attachment behavior, while individual sorters can override the path template for specific item types.
...
- Good for: Guaranteed uniqueness, tracing files to messages
...
- Use `eventIdPrefix` when you want to trace files back to messages easily
