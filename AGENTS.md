...
- `src/server.ts` is the Hono HTTP entrypoint; depends on `config.ts` (env + TOML), `storage.ts` (SQLite/filesystem), and `sorters.ts` (sorter/routing logic). Types live in `src/types.ts`.
...
- Focus coverage on sorters (`src/sorters.ts`), storage paths (`src/storage.ts`), and auth handling.
